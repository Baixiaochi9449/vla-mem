...
class Pi05V2DeepseekMemoryEncoder(nn.Module):
    def __init__(self, config: PI05V2DeepseekConfig, vision_dim: int, expert_dim: int):
        ...
        self.memory_dim = config.memory_expert_dim
        self.vision_to_expert_proj = nn.Linear(vision_dim, self.memory_dim)
        self.state_proj = nn.Linear(config.max_state_dim, self.memory_dim)
        self.camera_embedding = nn.Embedding(config.memory_max_cameras, self.memory_dim)
        self.slot_embedding = nn.Embedding(len(config.memory_history_deltas), self.memory_dim)
        self.delta_mlp = nn.Sequential(
            nn.Linear(1, self.memory_dim),
            nn.SiLU(),
        ...
        self.output_proj = nn.Identity() if self.memory_dim == expert_dim else nn.Linear(self.memory_dim, expert_dim)
    ...
    def forward(
        ...
        image_summaries = image_summaries.view(batch_size, num_steps, num_cameras, self.memory_dim)
        ...
        view_inputs = image_summaries.view(batch_size * num_steps, num_cameras, self.memory_dim)
        ...
        timestep_tokens = self._masked_mean(encoded_views, ~view_padding).view(batch_size, num_steps, self.memory_dim)
